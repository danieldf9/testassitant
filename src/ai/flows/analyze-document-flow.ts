...
Instructions:
1.  **Identify Major Features/Themes as Epics:** These are large bodies of work. Each epic should have a clear summary and a comprehensive description explaining its overall goal, benefits, and scope.
2.  **Break Down Epics into User Stories or Tasks:**
    *   **Stories:** For user-centric features. A story **must** clearly state who wants what and why (e.g., "As a [user role], I want [feature] so that [benefit]"). The description **must be detailed enough** for a developer to understand the feature's scope and purpose without needing to refer back to the main document extensively. **Crucially, for every Story, generate a detailed list of Acceptance Criteria.** These criteria should be specific, measurable, achievable, relevant, and testable. Format acceptance criteria clearly, for example, under a heading like "Acceptance Criteria:" followed by a numbered or bulleted list. Example:
        "Acceptance Criteria:
        1. User can X.
        2. System does Y when Z occurs.
        3. Performance of action A is under N seconds."
    *   **Tasks:** For specific pieces of work that are not necessarily user-facing but are required to deliver a feature or epic. Descriptions **must be technically detailed**, outlining what needs to be done. If applicable, include **detailed Acceptance Criteria** for tasks as well, especially if they involve verifiable outcomes or technical deliverables. Use the same formatting for Acceptance Criteria as for Stories.
    *   **Bugs:** If the document describes existing issues or defects, create Bug tickets. The description should include steps to reproduce, actual result, and expected result.
3.  **Decompose Stories/Tasks into Sub-tasks:** These are smaller, actionable steps required to complete a story or task. Descriptions for sub-tasks should be concise and clearly state the work item. Avoid creating sub-tasks that are too granular (e.g., "Write a comment").
...
5.  **Ticket Details (Ensure comprehensiveness for development):**
    ...
    *   \\\`description\\\`: A **comprehensive and self-contained description** derived from the document, detailed enough for a developer to begin work. For Stories and Tasks, this **must include clearly formatted Acceptance Criteria** as specified in point 2. For Epics, it should detail the overall feature. For Bugs, it should contain reproduction steps, actual vs. expected results.
...
6.  **Completeness & Detail:** Be thorough. Capture all distinct pieces of work. Prioritize detail and clarity in descriptions and acceptance criteria. If the document is vague on a specific point needed for a ticket, make reasonable assumptions based on common practices for the described feature and clearly state "Assumption: ..." within the ticket description. The goal is to produce tickets that are as development-ready as possible.
